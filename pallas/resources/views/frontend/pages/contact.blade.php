...
<section class="bg-img1 txt-center p-lr-10 p-tb-92" style="background-image: url('{{ asset('storage/img/bg-01.jpg') }}');">
    <h2 class="stext-100 cl2 txt-center">
        Liên hệ
    </h2>
</section>
            ...
            <div class="size-210 bor10 p-lr-70 p-t-55 p-b-70 p-lr-15-lg w-full-md">
                <form name="contactForm" ng-submit="submitContactForm()" novalidate>
                    <h4 class="mtext-105 cl2 txt-center p-b-30">
                        Gửi lời nhắn cho shop Hoa tươi Pallas
                    </h4>
                        ...
                        <ul>
                            ...
                            <li><span class="error" ng-show="contactForm.message.$error.maxlength">Lời nhắn phải <= 250 ký tự</span> </li> </li> </div> <!-- Div Thông báo validate hợp lệ Chỉ hiển thị khi các validate trong form `contactForm` không hợp lệ=> contactForm.$valid = true
                                        ...
                                        <div class="alert alert-success" ng-show="contactForm.$valid">
                                            Thông tin hợp lệ, vui lòng bấm nút <b>"Gửi liên hệ"</b> để gửi mail đến Quản trị viên<br />
                                        ...
                                        <!-- Nút submit form -->
                                        <button type="submit" class="flex-c-m size-121 bg3 bor1 hov-btn3 p-lr-15 trans-04 pointer" ng-disabled="contactForm.$invalid">
                                            <b style="color:white;">Gửi liên hệ</b>
                                        </button>
                </form>
            </div>
...
<script>
    ...
    app.controller('contactController', function($scope, $http) {
        ...
        $scope.submitContactForm = function() {
            // kiểm tra các ràng buộc là hợp lệ, Gửi AJAX đến action 
            if ($scope.contactForm.$valid) {
                ...
                // sử dụng service $http của AngularJS để Gửi request POST đến route `frontend.contact.sendMailContactForm`
                $http({
                    url: "{{ route('frontend.contact.sendMailContactForm') }}",
                    method: "POST",
                    data: JSON.stringify(dataInputContactForm)
                }).then(function successCallback(response) {
                    // Gửi mail thành công, thông báo cho khách hàng biết
                    swal('Gửi mail thành công!', 'Chúng tôi sẽ trả lời Quý khách trong thời gian sớm nhất. Xin cám ơn!', 'success');

                }, function errorCallback(response) {
                    // Gửi mail không thành công, thông báo lỗi cho khách hàng biết
